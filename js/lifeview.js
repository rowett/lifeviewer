...
(function() {
	...
	View.prototype.copyPatternTo = function(pattern) {
		...
		for (y = 0; y < copyHeight; y += 1) {
			...
			// check for multi-state view or Generations or LTL
			...
			if (me.statsOn) {
				...
				if (me.anythingAlive) {
					...
					if (me.engine.counter === 0) {
						...
						// check for Generations rule  TBD LTL?
	...
	View.prototype.startViewer = function(patternString, ignoreThumbnail) {
		...
		if (pattern) {
			...
			// read the number of states (Generations or LTL)
		...
		if (this.multiStateView) {
			...
			// create the colour grid if not Generations or LTL rule
			if (this.engine.multiNumStates === -1) {
